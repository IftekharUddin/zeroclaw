...
impl SopPane {
        ...
        match mouse.kind {
            MouseEventKind::Down(MouseButton::Left) => {
                ...
                if self.run_btn_rect.is_some_and(|r| in_rect(col, row, r)) {
                    if !self.read_only && self.selected_name().is_some() {
                        self.start_run_payload().await;
                    }
                    return;
                }
                if self.resume_btn_rect.is_some_and(|r| in_rect(col, row, r)) {
                    if !self.read_only && self.selected_resume_target().is_some() {
    ...
    fn render_controls(&mut self, f: &mut Frame, area: Option<Rect>) {
        ...
        let run_enabled = !self.read_only && self.selected_name().is_some();
        let resume_enabled = !self.read_only && self.selected_resume_target().is_some();

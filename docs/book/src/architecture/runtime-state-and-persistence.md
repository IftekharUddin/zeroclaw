...
| Gateway session incarnations | `zeroclaw_gateway::session_lifecycle::SessionLifecycle` | none | gateway `AppState` | Per-session lifecycle authority serializes admission, handshake/state metadata, transcript completion, REST writes, and delete; generations reset on daemon restart | This is the sole in-process owner of gateway incarnation, finalization, and concurrent failure generations. Runtime RPC and generic session tools must not mutate the `gw_` namespace because they cannot participate in this authority; use the authenticated gateway session-message API for external writes. |
| Gateway transcript completeness | `SessionBackend` durable intent marker | SQLite `session_metadata.transcript_incomplete`; JSONL `<session>.jsonl.incomplete` | Gateway lifecycle authority checks the backend marker before mutation | The marker is persisted before transcript writes and cleared only after all writes succeed; it survives reload and restart | Backend existence or a successful load cannot prove that a partial transcript was repaired. A marked transcript remains closed to new writers until successful delete/recreate removes both the transcript and marker. |
